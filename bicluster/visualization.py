...
def plot_biclusters_heatmap(adata: sc.AnnData, biclusters: list, save_path=None):
    ...
    for bicluster in biclusters:
        ...
        if len(bicluster.cell_index) > 0 and len(bicluster.gene_index) > 0:
            subset = mat[bicluster.cell_index, :][:, bicluster.gene_index]
            all_data.extend(subset.flatten())
    
    global_vmin = 0
    global_vmax = np.percentile(all_data, 95) if all_data else 1
...
def create_cell_sankey_diagram(adata, source_key="louvain", target_key="biclusters", 
 ...
 : str
    ...
    louvain_colors = generate_distinct_colors(len(unique_louvains))
