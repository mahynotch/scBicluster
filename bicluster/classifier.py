...
class GSEAConfidenceClassifier:
    ...
    def annotate_single_cell(
        ...
        ranks = expr_series.rank(method="average", ascending=True).values  # smallest value = rank 1
    ...
    def _get_enrichment_features(self, adata: sc.AnnData):
        ...
        for i, cell_expression in enumerate(X):
            for j in range(len(self.biclusters)):        
                score = self.annotate_single_cell(adata.var_names, cell_expression, self.positive_genes[j], self.negative_genes[j])[0]
                feature_matrix[i, j] = score
