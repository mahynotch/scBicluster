...
def nonsparseness_score(adata, bicluster) -> float:
    ...
    """
    mtx = adata.X[bicluster.cell_index, :][:, bicluster.gene_index].toarray()
    indicate = mtx > 0
...
def relevance_score(adata, bicluster) -> float:
    gene_mtx = adata.X[:, bicluster.gene_index].toarray()
    gene_var = np.var(gene_mtx, axis=0) 
    in_mtx = adata.X[bicluster.cell_index, :][:, bicluster.gene_index].toarray()
